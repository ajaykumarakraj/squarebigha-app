...
//   console.log('res', property.media && property.media[0] ? property.media[0].file_url : null);
  ...
  const location =
    property.location ||
    [property.locality, property.property_city]
      .filter(Boolean)
      .join(', ') ||
    'Location not available';

  // YOUR API HAS primary_media
const media = Array.isArray(property.media)
  ? property.media.filter(item => item?.file_url)
  : [];
    ...
    >
      {media.map((item, index) => (
        <Image
          key={item.id || index}
          source={{
            uri: `https://api.squarebigha.com/${item.file_url}`,
          }}
          style={styles.image}
          resizeMode="cover"
        />
  ...
  <View style={styles.heroActions}>
    ...
    >
      <Feather
        name="heart"
        size={20}
        color={saved ? '#B45E55' : '#18201D'}
        fill={saved ? '#B45E55' : 'none'}
      />
    </Pressable>
